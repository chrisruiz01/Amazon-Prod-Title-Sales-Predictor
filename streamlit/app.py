...
def preprocess(text):
    ...
    try:
        ...
        if sequences is None or sequences.shape[0] != 1 or sequences.shape[1] != max_text_length:
            raise ValueError("Vectorizer output is invalid")
...
def get_predictions(text):
    try:
        ...
        if X is None or X.shape[0] != 1:
            raise ValueError("Input to model is invalid")
        y = model.predict(X)
        print("Model prediction:", y)
        predictions = y.tolist()
        if not predictions or len(predictions) != 1 or not predictions[0]:
            raise ValueError("Predictions list is empty or malformed")
        return predictions
    except Exception as e:
